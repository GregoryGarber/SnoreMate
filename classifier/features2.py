...
class FeatureExtractor():
    ...
    def _compute_formant_features(self, window):
        ...
        # Define the bin edges and compute the histogram of the formants
        bin_nums = np.linspace(0, 5500, num=len(formants))
        hist, _ = np.histogram(formants, bins=bin_nums, range=(0,5500))
    ...
    def _compute_delta_coefficients(self, window, N=2):
        ...
        n_frames, n_coeff = mfcc_feats.shape

        # Set n to 2 as per the equation
        n = 2

        # Compute the denominator
        denominator = sum([i**2 for i in range(1, n+1)]) * 2

        # Create an empty 75 x 13 array for delta coefficients
        delta_feats = np.zeros((n_frames - 2*n, n_coeff))

        # Iterate over all windows
        for t in range(n, n_frames-n):
            # Compute the numerator
            numerator = np.zeros(n_coeff)
            for i in range(1, n+1):
                numerator += i * (mfcc_feats[t+i] - mfcc_feats[t-i])
            # Divide the numerator by the denominator
            delta_feats[t-n] = numerator / denominator

        # Flatten the delta coefficients array to get a 1D array
        delta_feats = delta_feats.flatten()
